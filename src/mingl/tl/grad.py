...
class Neighborhoods:
    def __init__(
        self, cells, ks, cluster_col, sum_cols, keep_cols, neigh, X="X:X", Y="Y:Y", reg="Exp", add_dummies=True
    ):
    ...
    def k_windows(self, distance_max="none"):
        ...
        out_dict = {}
        max_distance = None if distance_max == "none" else distance_max
        ...
        for k in self.ks:
            for (neighbors, sorted_dists), job in zip(tissues, tissue_chunks):
                ...
                if max_distance is not None:
                    ...
                    print(
                        f"{tissue_name}:{k}--{(no_cells_masked > 0).sum()}/{len(no_cells_masked)} cells had cells excluded.  Avg excluded={avg_cells_excluded}"
                    )

                # shared with KNN2 (mingl.tl.knn2): the same helper thresholds
                # a k-window sum by distance, so both entry points agree.
                summed = _sum_windows(window, dists_k, max_distance)
        ...
        for k in self.ks:
            window = pd.concat(
                [
                    pd.DataFrame(out_dict[(exp, k)][0], index=out_dict[(exp, k)][1].astype(int), columns=self.sum_cols)
...
def mingl_neighborhoods_scverse(
    ...
    *,
    ...
    ``tu1`` and ``tu2`` name the two tissue-unit probability columns compared
    (for example, "Inner Follicle" and "Outer Follicle" in the manuscript);
    there is no generic default, so a workflow config supplies them explicitly.
